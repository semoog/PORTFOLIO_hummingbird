...
angular.module("meanmail").controller("mailCtrl", function ($scope, $http, user, $state, hotkeys) {
    ...
    $scope.getMail = (label) => {
        ...
        return $http({
            method: 'GET',
            url: 'http://hummingbird.email:80/getMail/' + label
        }).then((response) => {
    ...
    $scope.sendMail = (headers_obj, message) => {
      return $http({
          method: 'POST',
          url: 'http://hummingbird.email:80/sendMail/',
    ...
    $scope.watchMail = () => {
      return $http({
          method: 'GET',
          url: 'http://hummingbird.email:80/watchMail/'
      }).then((response) => {
    ...
    $scope.trashMail = (messageId) => {
      return $http({
          method: 'POST',
          url: 'http://hummingbird.email:80/trashMail/',
          data: {
            messageId: messageId
          }
      }).then((response) => {
    ...
    $scope.removeLabel = (messageId, label) => {
      return $http({
          method: 'POST',
          url: 'http://hummingbird.email:80/removeLabel/',
          data: {
            messageId: messageId,
            label: label
          }
    ...
    $scope.addLabel = (messageId, label) => {
      return $http({
          method: 'POST',
          url: 'http://hummingbird.email:80/addLabel/',
          data: {
            messageId: messageId,
            label: label
          }
